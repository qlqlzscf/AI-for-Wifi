%% X410 Q-Learning Offline Training (Beta-State Q-table, fixed link)
...
fprintf("X410 Q-Learning Training (beta-state) | N=%d | saveInterval=%d\n", N, saveInterval);
fprintf("  Q-table file: %s\n", qtableFile);
fprintf("  TX=%g dB RX=%g dB (fixed, no channel variation)\n", gainDataTx, gainDataRx);
...
%% Helpers
bwDec2Str = @(bwdec) ternary(bwdec==2, "CBW20", ternary(bwdec==4, "CBW40", ternary(bwdec==8, "CBW80", "CBW160")));
...
for n = 1:N
    ...
    % Resample from hwSR to WLAN rate
    ...
    while Nt - windowStartNt + 1 >= WQLearning
        ...
        if length(ctlinfoList) >= 25
            dispPER = sum(ctlinfoList(end-24:end) == 2) / 25 * 100;
        else
            dispPER = NaN;
        end
        fprintf("[Q-update %03d] n=%d/%d | PER25=%.1f%% beta1=%.1f(s%d) beta2=%.1f(s%d) | rew=%.2f MB/s eps=%.3f\n", ...
            kQL-1, n, N, dispPER, beta1new, currentState1, beta2new, currentState2, reward_k/1e6, epsilon);
    ...
    if mod(n, saveInterval) == 0
        ...
        elapsed = toc;
        fprintf("--- [SAVE] n=%d/%d (%.1f%%), Q-updates=%d, elapsed=%.0fs ---\n", ...
    ...
    if mod(n, 100) == 0
        elapsed = toc;
        fprintf("--- Progress: %d/%d (%.1f%%), Q-updates=%d, MCS=%d, AMPDU=%d, BWdec=%d, elapsed=%.0fs ---\n", ...
            n, N, 100*n/N, kQL-1, MCS, numampdunew, ChannelBWnewDec, elapsed);
    end
end
...
figure('Name', 'Q-Learning Training Dashboard (Train1)', 'Position', [30 50 1600 900]);
...
if length(rewardHistory) > 10
    ...
    try
        ...
        ma = filter(ones(win,1)/win, 1, rewardHistory(win:end)/1e6);
        plot(rewardIter(win:end), ma, 'r-', 'LineWidth', 2);
    end
end

subplot(3,4,2);
plot(rewardIter, beta1History, 'r-', 'LineWidth', 1.5); grid on;
xlabel('Iteration'); ylabel('beta1'); ylim([0.1 0.9]);
title(sprintf('beta1: %.1f -> %.1f', beta1History(1), beta1History(end)));

subplot(3,4,3);
plot(rewardIter, beta2History, 'b-', 'LineWidth', 1.5); grid on;
xlabel('Iteration'); ylabel('beta2'); ylim([1.1 2.5]);
...
sgtitle(sprintf('Q-Learning Training beta-state beta1:%.1f->%.1f beta2:%.1f->%.1f updates=%d', ...
...
Nbyte = 0; nPkts = size(rewardState, 2);
TdataSec = zeros(1, nPkts); TprotocolSec = (84 + 10) * 1e-6;
...
for i = 1:nPkts
    ...
    if rewardState(1, i) == 1, Nbyte = Nbyte + 2304 * ampduVal; end
end
...
if ~visit1(actRow1, prevState1)
    ...
    if any(nextVisited), nextMax = max(Q1(nextVisited, currentState1)); else, nextMax = 0; end
...
if ~visit2(actRow2, prevState2)
    ...
    if any(nextVisited), nextMax = max(Q2(nextVisited, currentState2)); else, nextMax = 0; end
...
if rand(1) > epsilon
    ...
    row1 = randi(numel(actionSpace)); row2 = randi(numel(actionSpace));
end
