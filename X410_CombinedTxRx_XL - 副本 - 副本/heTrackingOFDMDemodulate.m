...
if strcmp(cfgTrack.PilotTracking,'Joint')
    ...
    if cfgTrack.PilotTrackingWindow>numOFDMSym
        cfgTrack.PilotTrackingWindow = double(numOFDMSym-(rem(numOFDMSym,2)==0));
    end
...
N = double(ofdmInfo.FFTLength);             % FFT length is samples
Ng = double(ofdmInfo.CPLength);   % Number of samples in GI
Ns = double(N+Ng);                       % Number of samples per symbols
kst = double(ofdmInfo.ActiveFrequencyIndices); % Indices of all active subcarriers
kd = double(kst(ofdmInfo.DataIndices));  % Indices of data carrying subcarriers
kp = double(kst(ofdmInfo.PilotIndices)); % Indices of pilot carrying subcarriers
Nd = double(numel(kd));             % Number of data carrying subcarriers
Np = double(numel(kp));             % Number of pilot carrying subcarriers
Nr = double(size(chanEst,3)); % Number of receive antennas

n = double(0:numOFDMSym-1);
z = double(2+numHESIGA+numHESIGB); % Pilot symbol offset
...
for n = 1:numOFDMSym
    ...
    idx = double((n-1)*Ns + (1:Ns) + skipdup);
    ...
    perridx = double(max((n-double(cfgTrack.PilotTrackingWindow)+1),1):n);
...
pilotGain = movmean(abs(perr).',double(cfgTrack.PilotTrackingWindow)/2);

% The averaging causes a delay which we correct for before applying
% correction
delay = double(cfgTrack.PilotTrackingWindow-1)/2;
...
% for delay
keepIdx = setdiff(double(1:numOFDMSym),double(2:2:cfgTrack.PilotTrackingWindow)); % Remove even averages at start when growing window
